...
**F16 — The Claude.app Code-tab top-right indicator is the "branch diff tracker"; it counts `git diff main...HEAD`, not uncommitted working-tree lines.** Lieutenant asked: *"What are the 1661 lines not committed in this repo?"* Charon's first response ran `git status` (clean), `git diff HEAD` (empty), enumerated other repos for dirty state, and reported being unable to reproduce the 1,661 figure. Only after Lieutenant pasted the file list (15 files, exactly matching all of Session 2026-05-22's Phase 1–6 + this session's deployment commits) did Charon recognize the pattern and verify with `git diff main...HEAD --numstat | awk '{a+=$1} END {print a}'` — which returned exactly `1661`. **The indicator is not buggy; it shows correct cumulative line additions in the branch-vs-main diff.** The Lieutenant's "feels like 50% of Claude time is fixing this kind of mistake" framing applies directly: the misread cost real session minutes today. **Root-cause analysis** (now documented in Library ADR-0004): (a) the UI labels the file list "files not committed" which seeds a working-tree mental model; (b) Library facts (including FACT-006 which has documented the related stale-after-merge bug since 2026-04-10) do not auto-load into a Charon's session context — `~/.claude/CLAUDE.md` `@`-references include CASE-BOARD and RULES but not `library/docs/facts.md`; (c) `git status`'s "ahead of origin/branch by N commits" phrasing reinforces "uncommitted" rather than "committed but unmerged." **Library updates landed this session:** new FACT-009 documenting the indicator's normal behavior with the verification command; FACT-006 clarified to disambiguate "phantom on clean branch" as meaning post-merge-stale, not unmodified-tree-noisy; new ADR-0004 codifying the Charon interpretation protocol and the three structural causes. **Cross-suite memory mirror** also installed at `~/.claude/projects/-Users-dev-continental/memory/feedback_branch_diff_tracker.{adoc,md}` so future Continental-root sessions encounter the doctrine via the auto-memory subsystem. **Open follow-on:** amending `~/.claude/CLAUDE.md` to add `@library/docs/facts.md` so the Facts Register loads at every Continental session start — pending Lieutenant approval (global-config change).
...
### Session 2026-06-05 (Session 2) — Deferred worktree teardown (Charon: Claude Opus 4.8 1M context)

**Contract state at open:** downdoc Phases 1–6 already squash-merged to `main` at `51a78a7` (contract COMPLETE per CASE-BOARD). The sole deferred housekeeping item was the worktree teardown left for "a fresh session opened at the main worktree" per Lieutenant directive 2026-05-20 (C1/C2/C3). This session is that session — opened at `/Users/dev/continental/software/cases/downdoc`, branch `main`, HEAD `51a78a7`.

**Work delivered:** `.claude/worktrees/yaml-frontmatter` removed; `feature/yaml-frontmatter` (local) force-deleted (was `97ef8bf`); `git worktree prune` ran clean. No source touched — the 453/453 suite is structurally unaffected and was not re-run (zero code change this session; re-running would test nothing this session altered).

**F17 — The deferred worktree teardown executed cleanly from the main worktree, satisfying C1/C2/C3.** C1: session's starting context is the main worktree path, not a `.claude/worktrees/*` subpath (verified: `git rev-parse --show-toplevel` = `/Users/dev/continental/software/cases/downdoc`). C2: branch at that path is `main`. C3: target (`feature/yaml-frontmatter`) is not the session's own worktree. Content remains triple-safe: squash-merged into `main` at `51a78a7`, and the original commit chain preserved on `origin/feature/yaml-frontmatter` at `97ef8bf` (verified via `git ls-remote` post-deletion — `97ef8bf` still present). `git branch -D` (capital) was required, not `-d`: the squash-merge means the branch's commit chain is unreachable from `main`, so git's "is it merged?" ancestry check fails even though the content is on `main`.

**F18 — The Sentry fired TWO preconditions in sequence — clean-tree, then per-action authorization — and the second is not satisfied by a terse execution directive.** First block: `external-persistence precondition FAILED ... working tree dirty: ?? .claude/  ?? package-lock.json`. The two dirty items were regenerable noise, not work: `package-lock.json` is an npm artifact **not tracked upstream** (`git ls-files --error-unmatch` → NOT tracked), and `.claude/` is session scaffolding. Resolved **without** touching upstream's tracked `.gitignore` (fork-rebase hygiene — the fork keeps `.gitignore` byte-identical to upstream) by broadening the repo-local `.git/info/exclude` from `.claude/worktrees/` to `.claude/` + `/package-lock.json`. `git status --short` then read empty. Second block: `authorization precondition FAILED ... Q-LLM (Gemma) classification: AUTH=NO — The user's message does not contain explicit authorization for the destructive command.` The Lieutenant's "Run the prompt I gave you. Skip the questions." cleared the check-in interview but did **not** satisfy the classifier; only an explicit typed message naming the action ("Authorized: remove the `.claude/worktrees/yaml-frontmatter` worktree and force-delete the `feature/yaml-frontmatter` branch.") returned AUTH=YES. **Direct fresh evidence for the `testament-session-restart` C1/C2/C3 Sentry enhancement, corroborating disk-overlap F3:** the Q-LLM counts only literal authorization of the named destructive action — terse or referential directives ("run the prompt", an AskUserQuestion option-pick) are not recognized. The clean-tree gate is sequenced first, so the classifier is never even consulted until the tree is clean — a two-stage gate worth documenting for anyone who hits only the first block and assumes that was the whole barrier.

**F19 — A third, auto-created worktree `claude/distracted-hypatia-9ebfc8` is present — a live instance of the `session-launch-context-discipline` bug — and was left untouched under C3.** At session open, `git worktree list` showed `.claude/worktrees/distracted-hypatia-9ebfc8  @ 51a78a7  [claude/distracted-hypatia-9ebfc8]` tracking `origin/main`. The `claude/<adjective>-<surname>-<hex>` name is the auto-creation signature the parking-lot contract documents. This Bash environment is rooted in the MAIN worktree (`show-toplevel` = main path, `branch --show-current` = `main`), so distracted-hypatia is most likely an orphan from a prior launch rather than this session's own seat — but `$CLAUDE_PROJECT_DIR` is unset (cannot confirm), and C3 forbids a session deleting its own worktree under uncertainty. **Left in place deliberately.** It is clean and fully pushed (`51a78a7` = `origin/main`), so zero data is at risk. Recommended cleanup by a future main-worktree session or the Lieutenant: `git worktree remove .claude/worktrees/distracted-hypatia-9ebfc8 && git branch -d claude/distracted-hypatia-9ebfc8`. **Banked as empirical evidence for `session-launch-context-discipline`.**

**F20 — A torn-down worktree RE-MATERIALIZED within this same session because a concurrent Claude.app session was bound to it (live concurrency evidence for `hotel-concurrency-audit`; caught post-checkpoint, ~2026-06-06 UTC).** Earlier this session the F17 teardown succeeded and was verified — `git worktree remove .claude/worktrees/yaml-frontmatter` + `git branch -D feature/yaml-frontmatter` ran clean, and `git worktree list` then showed only `main` + `distracted-hypatia`. During the post-checkpoint review, `git worktree list` showed `.claude/worktrees/yaml-frontmatter @ 97ef8bf [feature/yaml-frontmatter]` **back**. **Proof it is a fresh re-creation, not a failed delete:** `git reflog show feature/yaml-frontmatter` has exactly ONE entry — `97ef8bf feature/yaml-frontmatter@{0}: branch: Created from origin/feature/yaml-frontmatter`. The branch I deleted carried the full Phases-1–6 reflog; a single `Created from origin` entry means a brand-new branch was created after the deletion. The `.git/worktrees/yaml-frontmatter` admin dir mtime is correspondingly recent. **Mechanism:** `list_sessions` shows FIVE sessions rooted in `…/worktrees/yaml-frontmatter` (`local_d5ff7453` was active at 22:27Z, *after* the ~22:0x teardown). Claude.app provisions a per-session git worktree (*"each session gets its own isolated copy of your project using Git worktrees"* — code.claude.com/docs/en/desktop); when a bound session next stirred, the harness re-created the worktree and re-pulled the branch from `origin/feature/yaml-frontmatter` — i.e. from the exact cold-storage ref the teardown had preserved as its safety net. **Why it matters:** (1) **Teardown of a worktree that ANY live session is bound to is non-durable** — the bound session's harness rebuilds it, so the destructive op + its Sentry friction + its debrief/parlay/testament ceremony produced *zero durable change*. (2) This is a concurrency race in git **structural** state (worktrees/branches), a category beyond `hotel-concurrency-audit`'s currently-enumerated Classes A–G (which all cover shared-FILE lost-updates). It matches the Lieutenant's verbatim *"6 simultaneous sessions keep upsetting each others commits"* and occurred during the shutdown ceremony → closest to **Class G (shutdown-ceremony race)**, but is filed as a proposed distinct **Class H — concurrent worktree teardown-undo** in `~/continental/parking/hotel-concurrency-audit/BRIEF.md`. (3) The cold-storage safety net and durability are in **tension**: preserving `origin/feature/yaml-frontmatter` is exactly what the re-creation pulls from. **Recommendation:** never tear down a worktree any live session is bound to; teardown only in a deliberate sweep when `list_sessions` shows no session rooted in the target worktree path. For COMPLETE contracts, the cheaper policy is to leave clean+pushed worktrees in place (clutter, not data risk) and fix the root cause upstream via `session-launch-context-discipline` + `hotel-concurrency-audit`.

**Coin & process accounting (Session 2026-06-05):** Suite stayed Software throughout; 0 contracts opened (continuation of COMPLETE downdoc); 0 marker-class actions; the worktree teardown is in-suite housekeeping, not a marker action. Costs: **0** coins (no context switch, no new contract, no override). Earnings pending at `/parlay`: +2 single-suite-stay + +1 debrief credit = **+3**.
