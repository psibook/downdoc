...
You can emit a YAML frontmatter block at the top of the Markdown output using the `--frontmatter-vendor` option.
See <<yaml-frontmatter>> below for the attribute convention.

[,console]
----
$ npx downdoc --frontmatter-vendor=claude README.adoc
----
...
The supported options are `attributes` (an object of runtime AsciiDoc attributes) and `frontmatterVendor` (a vendor namespace string, an array of strings, or the special value `'all'`; see <<yaml-frontmatter>> below).

[,js]
----
downdoc(asciidoc, { frontmatterVendor: 'claude' })
----
...
When invoked with a `frontmatterVendor` API option or `--frontmatter-vendor` CLI flag, downdoc emits a YAML frontmatter block at the top of the Markdown output.
The block is populated from document-header attributes whose names match the convention `frontmatter-<vendor>-<artifact>-<field>`.
Each matching attribute is stripped of its `frontmatter-<vendor>-<artifact>-` prefix, the remaining kebab-case is translated to camelCase, and the result becomes a YAML key.

For example, the following AsciiDoc input:
...
with `--frontmatter-vendor=claude` (or `{ frontmatterVendor: 'claude' }` in the API) produces:
...
The vendor selector accepts:

* a vendor name (e.g., `claude`), in which case only attributes in that namespace are emitted
* the special value `all`, in which case every matching namespace is emitted, grouped alphabetically by vendor
* the flag specified multiple times (CLI) or an array of strings (API) for an explicit list of vendors

When the option is not specified, no frontmatter is emitted (even if matching attributes are present in the document); the feature is opt-in.
Attribute values may span multiple lines using AsciiDoctor's `\` line-continuation; continuation lines are joined with a single space.
Only attributes that appear in the document header (before the first blank line after the document title) participate; matching attribute entries elsewhere in the body are ignored.
