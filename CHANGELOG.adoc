...
* Support emitting a YAML frontmatter block at the top of the Markdown output from AsciiDoc document-header attributes matching the pattern `frontmatter-<vendor>-<artifact>-<field>`.
The feature is opt-in via the `frontmatterVendor` API option or `--frontmatter-vendor` CLI flag (single vendor, repeated flag for multiple vendors, or the special value `all` for every matching namespace).
The attribute name prefix is stripped and the remainder is translated from kebab-case to camelCase to form the YAML key.
Multi-line attribute values that use AsciiDoctor's `\` line-continuation are joined with a single space.
See the README's "YAML frontmatter" section for a worked example.

== 1.1.0-stable (2026-03-23)

=== Added

* Support link attribute on image macros (#13)
* Apply normal substitutions to section titles / discrete headings (#25)
* Support `indent` attribute with positive value on verbatim block (#32)

=== Changed

* Clean invalid characters from auto-generated ID (#30)

== 1.0.3-stable (2026-03-23)

=== Fixed

* Only consider first positional attribute when looking for alt text on image macro (#29)
* Apply passthrough semantics to contents of inline stem expression (#22)
