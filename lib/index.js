...
module.exports = function downdoc (asciidoc, { attributes: initialAttrs = {}, frontmatterVendor } = {}) {
  const attrs = Object.assign(new Map(Object.entries(Object.assign({}, ATTRIBUTES, initialAttrs))), { pass: [] })
  const fmVendors = frontmatterVendor === 'all'
    ? 'all'
    : frontmatterVendor
      ? (Array.isArray(frontmatterVendor) ? frontmatterVendor : [frontmatterVendor]).filter(Boolean)
      : null
  const frontmatter = fmVendors && (fmVendors === 'all' || fmVendors.length) ? new Map() : null
  const captureFrontmatter = (name, resolved, isInHeader) => {
    if (isInHeader && frontmatter && name.startsWith('frontmatter-')) {
      const segments = name.split('-')
      if (segments.length >= 4) {
        const vendor = segments[1]
        if (fmVendors === 'all' || fmVendors.includes(vendor)) {
          const fieldParts = segments.slice(3)
          const camelKey = fieldParts[0] + fieldParts.slice(1).map((s) => s[0].toUpperCase() + s.slice(1)).join('')
          const inner = frontmatter.get(vendor) || frontmatter.set(vendor, new Map()).get(vendor)
          inner.set(camelKey, resolved)
        }
      }
    }
    return resolved
  ...
  if (frontmatter) {
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].startsWith(':frontmatter-')) {
        while (i + 1 < lines.length && lines[i].endsWith(' \\')) {
          lines[i] = lines[i].slice(0, -2) + ' ' + lines.splice(i + 1, 1)[0].trimStart()
        }
      }
    }
  }
      ...
      while (true) {
        ...
        if (skipStack.length && (match = skipStack[skipStack.length - 1] || (line === 'endif::[]' && line))) {
          ...
          if (chr0 === '/' && line[1] === '/') {
            ...
            if (!(name in initialAttrs)) del ? attrs.delete(name) : attrs.set(name, captureFrontmatter(name, val && attributes.call(attrs, val), inHeader))
  ...
  if (frontmatter?.size) {
    const vendors = [...frontmatter.keys()].sort()
    const yamlLines = []
    for (const vendor of vendors) {
      for (const [key, value] of frontmatter.get(vendor)) yamlLines.push(key + ': ' + value)
    }
    return '---\n' + yamlLines.join('\n') + '\n---\n\n' + body
  }
  return body
