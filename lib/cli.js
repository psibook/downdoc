...
async function run (p = process) {
  const options = {
    ...
    'frontmatter-vendor': {
      type: 'string',
      multiple: true,
      desc: 'emit YAML frontmatter for the given vendor namespace, or "all"',
      hint: 'name',
    },
  ...
  const fmv = values['frontmatter-vendor']
  const frontmatterVendor = fmv?.length ? (fmv.length === 1 ? fmv[0] : fmv) : undefined
  const opts = attributes || frontmatterVendor
    ? { ...(attributes && { attributes }), ...(frontmatterVendor && { frontmatterVendor }) }
    : undefined
  if (!(await validateInputPath.call(p, inputPath))) return
  await convertFile.call(p, inputPath, opts, values.output)
  if (values.prepublish) await hideInputFile(inputPath)
}

function convertFile (inputPath, opts, outputPath = toOutputPath(inputPath)) {
  const write = (data) => (outputPath === '-' ? this.stdout.write(data) : fsp.writeFile(outputPath, data, 'utf8'))
...
function printUsage (options, error) {
  ...
  if (error) {
    ...
    Object.entries(options).forEach(([long, { short, hint, multiple, desc }]) => {
      const option = (short ? `-${short}, --${long}` : `--${long}`) + (hint ? ' ' + hint : '')
